...
def predict(X_img_path, knn_clf=None, model_path=None, DIST_THRESH=0.6):
    ...
    :param DIST_THRESH: (optional) distance threshold for face classification. the larger it is, the more chance
    ...
    return [{'name': pred, 'dims': loc} if rec else ("unknown", loc) for pred, loc, rec in zip(knn_clf.predict(faces_encodings), X_face_locations, are_matches)]
